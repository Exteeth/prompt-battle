...
export default function StageSidebar({ currentStageId, onSelectStage, isOpen, onClose }) {
  ...
  return (
    <>
      ...
      <aside className="fixed inset-y-0 left-0 z-40 w-80 sm:w-84 bg-white border-r border-slate-200 flex flex-col shadow-2xl animate-fade-in h-[100dvh] text-slate-900 font-prompt">
        {/* Header */}
        <div className="p-4 border-b border-slate-200 flex items-center justify-between font-kanit">
          <div className="flex items-center gap-2.5">
            ...
            />
            <div>
              <h2 className="text-sm font-extrabold text-slate-900 leading-tight">Prompt Battle</h2>
              <p className="text-[11px] text-slate-500 font-prompt">เลือกด่าน & ดูคำแนะนำโจทย์</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-3 border-t border-slate-200 bg-slate-50 font-prompt">
          <button
            onClick={() => {
              window.location.href = '/leaderboard';
            }}
            className="btn-vibrant-yellow w-full min-h-[44px] py-2.5 px-3 rounded-xl text-xs font-extrabold flex items-center justify-center gap-2 transition-all cursor-pointer text-amber-950"
          >
